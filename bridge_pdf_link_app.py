...
def _parse_diag_photo_nums(text):
    ...
    対応形式:
      - 「写真番号1-1」「写真番号1-2(1-1)」→ N-M形式（複数径間PDF）: M を採用
      - 「写真番号1」「写真番号5」           → 単独形式（単一径間PDF）: そのまま採用
    括弧内（前回番号）は無視する。
    """
    text = _normalize_text(text)
    # ノイズ除去
...
def run_process(input_path, output_path, dpi, log_cb, done_cb):
    ...
    try:
        ...
        photo_num_to_page = {}   # 写真番号 -> page_idx（先着優先）
        ...
        for didx in diag_pages:
            ...
            diagram_to_nums[didx] = nums
            log_cb(f"  p.{didx+1}: 写真番号 {sorted(nums)}")

        # ── 対応関係を構築（写真番号のみで照合） ───────────────────────────
        diag_to_photo_pages = defaultdict(list)
        for didx, nums in diagram_to_nums.items():
            seen = set()
